...
- hover
- go-to-definition
- go-to-type-definition (a value's declared type, unwrapping optional/container shells)
...
- inlay hints (inferred type for `var`/`lab`/`con` bindings without an explicit
  annotation)
- code lens (a "Run" action above `fun[] main`)
...
This is still primarily a V1 surface.
The editor only claims shipped V2-aware behavior for the checked-in
generic-routine, generic-type, constrained-generic, and protocol-standard
examples, and it does not claim broad rename outside the documented safe
classes or range formatting.
