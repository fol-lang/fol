...
fn test_cli_rejects_old_style_build_and_ordinary_dot_graph() {
    ...
    let success = run_fol(&[temp_root.to_str().expect("package path should be utf-8")]);
    assert!(
    ...
    let old_style = run_fol(&[temp_root.to_str().expect("package path should be utf-8")]);
    ...
    let ordinary = run_fol(&[temp_root.to_str().expect("package path should be utf-8")]);
...
fn test_cli_json_typecheck_errors_keep_structured_fields() {
    ...
    fs::write(temp_root.join("main.fol"), "var[sta] value: int = 1;\n")
    ...
    assert!(
        first["message"]
            .as_str()
            .is_some_and(|message| message.contains("static binding semantics")),
...
fn test_cli_typecheck_accepts_generic_types_through_semantic_check() {
    ...
    fs::create_dir_all(temp_root.join("src"))
        .expect("Should create temp generic-type fixture root");
    fs::write(
            temp_root.join("build.fol"),
...
fn test_cli_typecheck_rejects_standard_signature_mismatches_for_m2() {
    ...
    assert!(
        !output.status.success(),
        "CLI should reject standard signature mismatches in M2"
    );
    assert!(
...
fn test_cli_typecheck_rejects_standards_as_ordinary_types_for_m2() {
    ...
    assert!(
        !output.status.success(),
        "CLI should reject standards as ordinary types in M2"
    );
    assert!(
...
fn test_cli_typecheck_accepts_blueprint_and_extended_standards_for_m2() {
    ...
    assert!(
        output.status.success(),
        "CLI should accept blueprint and extended standards"
    );
    assert!(
...
fn test_cli_typecheck_rejects_duplicate_protocol_members_for_m2() {
    ...
    assert!(
        !output.status.success(),
        "CLI should reject duplicate protocol members in M2"
    );
    assert!(
...
fn test_cli_typecheck_rejects_unknown_standard_contract_headers_for_m2() {
    ...
    assert!(
        !output.status.success(),
        "CLI should reject unknown standard contract headers"
    );
    assert!(
...
fn test_cli_json_keyword_intrinsic_arity_failures_keep_structured_fields() {
    ...
    fs::create_dir_all(&temp_root).expect("Should create temp CLI keyword intrinsic arity fixture");
    fs::write(
        temp_root.join("main.fol"),
        "fun[] main(): bol = {\n    return check();\n};\n",
    )
    ...
    let diagnostic = diagnostics.iter().find(|diagnostic| {
        diagnostic["message"]
            .as_str()
            .map(|message| message.contains("check(...) expects exactly 1 argument(s) but got 0"))
            .unwrap_or(false)
    });
...
fn test_multi_file_parser_errors_keep_second_file_locations() {
    ...
    assert_eq!(loc.line, 1, "Second file should restart at line 1");
    assert_eq!(
        loc.column, 1,
