...
fn test_docs_book_and_agents_keep_current_v1_editor_contract() {
    ...
    assert!(
        lsp_book.contains("same-file local bindings, routine parameters, and current-package"),
...
#[test]
fn test_cli_json_v4_deallocation_boundary_keeps_structured_fields() {
    use std::fs;

    let temp_root = unique_temp_root("cli_json_intrinsic_v4_boundary");
    fs::create_dir_all(&temp_root).expect("Should create temp intrinsic V4 boundary fixture");
    let fixture = temp_root.join("main.fol");
    fs::write(
        &fixture,
        concat!(
    ...
    .expect("Should write intrinsic V4 boundary fixture");
    ...
    assert!(
        v4_error.is_some(),
        "Expected explicit V4 intrinsic boundary diagnostic in CLI JSON output, got: {json}"
    );
    assert!(
        v4_error
            .and_then(|diagnostic| diagnostic["location"].as_object())
            .is_some(),
        "Expected V4 intrinsic boundary diagnostic to keep a structured location, got: {json}"
    );
...
fn test_default_body_with_conformer_override_builds_end_to_end() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create default override fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_nil_error_shell_bindings_build_end_to_end() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create nil error fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_hardening_round_fixes_build_end_to_end() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create hardening fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_check_enforces_declared_capability_model() {
    ...
    fs::write(temp_root.join("build.fol"), build).expect("Should rewrite check-model build file");
    let output = run_fol_in_dir(&temp_root, &["code", "check"]);
    assert!(
        output.status.success(),
...
fn test_field_assignment_into_mutable_record_builds() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create field-assignment fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_records_with_container_fields_build_and_run() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create container-field fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_generic_variadic_routine_call_builds_end_to_end() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create generic variadic fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_routine_local_use_and_typ_ali_build_end_to_end() {
    ...
    fs::create_dir_all(temp_root.join("helpers")).expect("Should create local-use helper dir");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_loops_and_fallbacks_compute_correct_runtime_values() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create runtime-value fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_hardening_round6_generics_and_loops_compute_correct_runtime_values() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create round6 fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_record_defaults_and_positional_init_compute_correct_runtime_values() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create record-init fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_chained_fallbacks_compute_correct_runtime_values() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create chained-fallback fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_code_run_forwards_program_output() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create run-output fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_check_validates_artifact_roots_and_workspace_members() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create root-validation fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_default_arguments_survive_package_imports() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create defaults-import fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_emitted_aggregates_survive_floats_and_keyword_names() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create emission fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
...
fn test_pathological_nesting_rejects_instead_of_crashing() {
    ...
    fs::create_dir_all(temp_root.join("src")).expect("Should create nesting fixture dirs");
    fs::write(
            temp_root.join("build.fol"),
            concat!(
                "pro[] build(): non = {\n",
    ...
    fs::write(temp_root.join("src/main.fol"), deep).expect("Should write nesting fixture source");
