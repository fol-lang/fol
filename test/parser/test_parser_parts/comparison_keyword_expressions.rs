...
#[test]
fn test_function_return_parses_cast_expression() {
    let mut file_stream = FileStream::from_file("test/parser/simple_fun_cast_expr.fol")
        .expect("Should read cast-expression fixture");

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    let ast = parser
        .parse(&mut lexer)
        .expect("Parser should accept 'cast' expressions");
