...
use fol_parser::ast::{
    AstNode, AstParser, BindingPattern, CharEncoding, CommentKind, FloatSize, FolType,
    InquiryTarget, IntSize, Literal, Parameter, ParsedDeclScope, ParsedDeclVisibility,
    ParsedPackage, ParsedSourceUnit, ParsedTopLevel, QualifiedPath, SyntaxOrigin, TypeDefinition,
};
use fol_stream::FileStream;

/// Test-only helper: wraps `parse_script_package()` into `AstNode::Program`.
trait TestParse {
    fn parse(&mut self, lexer: &mut Elements) -> Result<AstNode, Vec<fol_diagnostics::Diagnostic>>;
}
...
fn program_root_nodes(declarations: &[AstNode]) -> Vec<&AstNode> {
    declarations.iter().collect()
}

fn only_root_routine_body_nodes(declarations: &[AstNode]) -> Vec<&AstNode> {
    assert_eq!(
        declarations.len(),
        1,
...
fn parse_package_from_file(path: &str) -> ParsedPackage {
    let mut file_stream =
        FileStream::from_file(path).expect("Should read parser package test file");
...
fn source_unit_nodes(source_unit: &ParsedSourceUnit) -> Vec<&AstNode> {
...
fn qualified_path_origin<'a>(package: &'a ParsedPackage, path: &QualifiedPath) -> &'a SyntaxOrigin {
    let syntax_id = path
        .syntax_id()
...
fn fol_type_origin<'a>(package: &'a ParsedPackage, typ: &FolType) -> &'a SyntaxOrigin {
    let syntax_id = match typ {
        FolType::Named { syntax_id, .. } => {
            syntax_id.expect("Named type should retain a syntax id in parsed-package mode")
        }
