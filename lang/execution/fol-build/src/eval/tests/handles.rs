...
fn temp_build_package(source: &str) -> (fol_testkit::TempFixture, PathBuf) {
    let package_root = fol_testkit::TempFixture::new("fol_build_eval_hdl");
    ...
    let cases = [
        (
            concat!(
                ...
                "    var app = graph.add_exe({ name = \"app\", root = \"src/main.fol\" });\n",
                "    var provider = graph.file_from_root(\"native/widget.o\");\n",
                "    app.add_c_import({ alias = \"widget\", header = \"native/widget.h\", provider = provider, provider_kind = \"object\" });\n",
                "    return;\n",
                "};\n",
            ),
        ...
        (
            concat!(
                ...
                "    var app = graph.add_exe({ name = \"app\", root = \"src/main.fol\" });\n",
                "    var header = graph.file_from_root(\"native/widget.h\");\n",
                "    app.add_c_import({ alias = \"widget\", header = header, provider = \"native/widget.o\", provider_kind = \"object\" });\n",
                "    return;\n",
                "};\n",
            ),
...
fn build_source_evaluator_rejects_invalid_c_import_kind_paths_and_duplicates() {
    let cases = [
        ...
        (
            ...
            "Object",
            "",
            "add_c_import config is invalid: 'provider_kind' must be one of object, static, shared, got 'Object'",
        ),
        (
            "native/widget.h",
            "native/widget.o",
        ...
        (
            ...
            concat!(
                ...
                "    app.add_c_import({ alias = \"widget\", header = other_header, provider = other_provider, provider_kind = \"object\" });\n",
            ),
            "artifact 'artifact:0' already imports a C namespace aliased 'widget'",
        ),
    ];
    ...
    for (header, provider, provider_kind, repeated, expected) in cases {
        let source = format!(
            concat!(
                ...
                "    app.add_c_import({{ alias = \"widget\", header = header, provider = provider, provider_kind = \"{provider_kind}\" }});\n",
                "{repeated}",
                "    return;\n",
                "}};\n",
            ),
    ...
    for (kind_field, expected) in [
        ...
        let source = format!(
            concat!(
                ...
                "    var header = graph.file_from_root(\"native/widget.h\");\n",
                "    var provider = graph.file_from_root(\"native/widget.o\");\n",
                "    app.add_c_import({{ alias = \"widget\", header = header, provider = provider{kind_field} }});\n",
                "    return;\n",
                "}};\n",
            ),
