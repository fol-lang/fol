...
pub fn evaluate_build_source(
    ...
    let mut resolved_inputs = request.inputs.options.clone();
    ...
    let first_output = executor
        ...
        .with_install_prefix(request.inputs.install_prefix.clone())
        .execute(&body)?;
    if first_output.operations.is_empty() {
        return Ok(None);
    }
    let first_result = evaluate_build_plan(&BuildEvaluationRequest {
        package_root: request.package_root.clone(),
        inputs: request.inputs.clone(),
        operations: first_output.operations,
    })?;

    // Execute a second time with declarations, defaults, and CLI overrides
    // resolved. This makes conditional graph construction deterministic and
    // ensures option handles cannot leak unresolved spellings into artifacts.
    let Some((executor, body)) = BuildBodyExecutor::from_file(build_path)? else {
        return Err(BuildEvaluationError::new(
            BuildEvaluationErrorKind::Internal,
            "build entry disappeared between evaluation passes",
        ));
    };
    let resolved_inputs = first_result
        .resolved_options
        .iter()
...
pub(super) fn build_evaluated_program(
    ...
    let mut step_bindings = exec_output
        ...
        .collect::<Vec<_>>();
    let executable_artifacts = result
        .graph
        .artifacts()
        .iter()
        .filter(|artifact| artifact.kind == crate::graph::BuildArtifactKind::Executable)
        .collect::<Vec<_>>();
    let test_artifacts = result
        .graph
        .artifacts()
        .iter()
        .filter(|artifact| artifact.kind == crate::graph::BuildArtifactKind::Test)
        .collect::<Vec<_>>();
            ...
            .any(|binding| binding.step_name == "build")
    {
        step_bindings.push(BuildRuntimeStepBinding::new(
            ...
            Some(executable_artifacts[0].name.clone()),
        ));
    }
    if test_artifacts.len() == 1
        && !step_bindings
            .iter()
            .any(|binding| binding.step_name == "test")
    {
        step_bindings.push(BuildRuntimeStepBinding::new(
            ...
            Some(test_artifacts[0].name.clone()),
        ));
    }
        ...
        .map(|artifact| {
            BuildRuntimeArtifact::new(
                ...
                match artifact.kind {
                    ...
                    crate::graph::BuildArtifactKind::Test => BuildRuntimeArtifactKind::Test,
                    crate::graph::BuildArtifactKind::Object => BuildRuntimeArtifactKind::Object,
                },
                artifact.root_module.clone(),
            )
            .with_fol_model(artifact.fol_model)
            .with_target_config(artifact.target.clone(), artifact.optimize)
            .with_c_imports(result.graph.c_imports_for(artifact.id).cloned().collect())
        })
        .collect::<Vec<_>>();
