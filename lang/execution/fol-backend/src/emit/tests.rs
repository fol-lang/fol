...
use crate::emit::{
    ...
    prepare_backend_build_paths, prepare_backend_runtime_build_dir, prepare_generated_build_dir,
...
fn build_and_run_fixture_with_cargo(source: &str) -> std::process::Output {
    ...
    let binary = cargo_build_generated_crate_for_profile(&crate_root, BackendBuildProfile::Release);
...
fn build_and_run_workspace(
    ...
    let lowered =
        lowered_workspace_from_entry_path_with_config(entry_path, package_config, resolver_config);
    let session = BackendSession::new(lowered);
...
fn auxiliary_plan_builds_ordered_rlibs_and_runnable_final_binary() {
    ...
    .unwrap();
    let generated_root = write_generated_crate(Path::new(&paths.build_root), &generated).unwrap();
...
fn rustc_generated_crate_builder_produces_runnable_release_binary() {
    ...
    let binary =
        build_generated_crate_with_rustc(&crate_root, &paths, &host, BackendBuildProfile::Release)
            .expect("rustc build");
...
fn emitted_generated_crate_remains_cargo_buildable_in_debug_mode() {
    ...
    let binary = cargo_build_generated_crate_for_profile(&crate_root, BackendBuildProfile::Debug);
    assert!(binary.exists());
...
fn bare_generic_forwarding_moves_unknowns_but_clones_copy_safe_callers() {
    ...
    for (name, should_clone) in [("owned", false), ("pointer", false), ("scalar", true)] {
        let source_local = main
            .locals
            .iter_with_ids()
            .find_map(|(local_id, local)| (local.name.as_deref() == Some(name)).then_some(local_id))
