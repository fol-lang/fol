...
pub fn parse_type_reference_text(source: &str) -> Result<ast::FolType, fol_diagnostics::Diagnostic> {
    ...
    let package = parser
        ...
        .map_err(|errors| errors.into_iter().next().unwrap_or_else(|| {
...
fn strip_type_syntax_ids(typ: ast::FolType) -> ast::FolType {
    match typ {
        ...
        ast::FolType::Pointer { target } => ast::FolType::Pointer {
