...
- recursive value edges remain rejected, including recursive generic
  instantiation through an inline container; V3 owned heap edges such as
  `opt @Node` are now legal and lower nominally to `Option<Box<Node>>`; the
  remaining boundary is pinned by `examples/fail_generic_recursive_m1m2`
